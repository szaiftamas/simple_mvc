// Select .toast-canvas that will wrap all .toast elements
const toastContainer = document.querySelector(".toast-container");

// Toast HTML content
const baseContent = 
`
  <div class="toast-header">
    <svg class="bd-placeholder-img rounded me-2" width="25" height="25" aria-hidden="true" preserveAspectRatio="xMidYMid slice">
      ###ICON###
    </svg>
    <strong class="me-auto">###TYPE###</strong>
    <small></small>
    <button type="button" class="btn-close" data-mdb-dismiss="toast" aria-label="Close"></button>
    </button>
  </div>
  <div class="toast-body">
    ###MESSAGE###
  </div>
`;

const successIcon = `<path fill="rgb(76, 175, 80)" d="M12.5 0C5.602 0 0 5.602 0 12.5S5.602 25 12.5 25 25 19.398 25 12.5 19.398 0 12.5 0zm-2.3 18.898l-5.5-5.5 1.8-1.796 3.7 3.699L18.5 7l1.8 1.8zm0 0"></path>`;
const infoIcon = `<path fill="rgb(3, 169, 244)" d="M12.504.035a12.468 12.468 0 100 24.937 12.468 12.468 0 000-24.937zM15.1 19.359c-.643.25-1.153.445-1.537.576-.384.134-.825.199-1.333.199-.775 0-1.381-.192-1.813-.57a1.832 1.832 0 01-.642-1.442c0-.227.015-.459.047-.693.03-.24.083-.504.154-.806l.802-2.835c.069-.272.132-.527.182-.77.048-.244.069-.467.069-.668 0-.36-.075-.615-.223-.756-.153-.144-.437-.213-.857-.213-.207 0-.422.036-.639.095a9.914 9.914 0 00-.56.184l.213-.874a19.777 19.777 0 011.51-.549 4.48 4.48 0 011.361-.23c.77 0 1.368.19 1.784.56a1.857 1.857 0 01.626 1.452c0 .122-.012.341-.04.652a4.44 4.44 0 01-.162.856l-.798 2.831a8.133 8.133 0 00-.176.775c-.05.288-.075.51-.075.66 0 .374.082.633.251.771.165.134.458.202.875.202.192 0 .412-.037.66-.1.243-.073.42-.127.531-.18zm-.144-11.483a1.901 1.901 0 01-1.343.518 1.93 1.93 0 01-1.352-.518 1.65 1.65 0 01-.562-1.258 1.688 1.688 0 01.562-1.266 1.914 1.914 0 011.35-.522c.524 0 .975.173 1.345.523a1.673 1.673 0 01.56 1.266 1.65 1.65 0 01-.56 1.257z"></path>`;
const warningIcon = `<path fill="rgb(255, 152, 0)" d="M24.585 21.17L13.774 3.24a1.51 1.51 0 00-2.586 0L.376 21.17a1.51 1.51 0 001.293 2.29h21.623a1.51 1.51 0 001.292-2.29zM12.49 8.714c.621 0 1.146.35 1.146.97 0 1.895-.223 4.618-.223 6.513 0 .494-.541.7-.923.7-.51 0-.94-.208-.94-.701 0-1.894-.223-4.617-.223-6.511 0-.62.51-.971 1.163-.971zm.015 11.734a1.225 1.225 0 01-1.225-1.226c0-.669.525-1.227 1.225-1.227.652 0 1.21.558 1.21 1.227 0 .652-.557 1.225-1.21 1.225z"></path>`;
const errorIcon = `<path fill="rgb(244, 67, 54)" d="M12.469.027c-3.332 0-6.465 1.301-8.824 3.653-4.86 4.86-4.86 12.777 0 17.636a12.392 12.392 0 008.824 3.653c3.336 0 6.465-1.301 8.824-3.653 4.863-4.859 4.863-12.777 0-17.636A12.417 12.417 0 0012.469.027zm5.61 18.086a1.137 1.137 0 01-.802.332c-.285 0-.582-.113-.8-.332l-4.008-4.008-4.008 4.008a1.137 1.137 0 01-.8.332c-.286 0-.583-.113-.802-.332a1.132 1.132 0 010-1.605l4.008-4.004L6.86 8.496a1.132 1.132 0 010-1.605 1.127 1.127 0 011.602 0l4.008 4.007 4.008-4.007a1.127 1.127 0 011.601 0c.45.449.45 1.164 0 1.605l-4.004 4.008 4.004 4.004c.45.449.45 1.164 0 1.605zm0 0"></path>`;

function showToast(type, message) {
  // Declare variable that will store toast instance
  let toastInstance;
  // Create empty <div> element
  const toast = document.createElement("div");

  // Add "toast" and "animate" classes to recently created <div>
  toast.classList.add("toast"); // , "fade", "show"
  toast.setAttribute("role", "alert");
  toast.setAttribute("aria-live", "assertive");
  toast.setAttribute("aria-atomic", "true");
  
  // Add HTML content from "toastContent" function parameter
  let thisContent = baseContent.toString();
  thisContent = thisContent.replace("###TYPE###", type).replace("###MESSAGE###", message)
  switch (type) {
    case "Success":
      thisContent = thisContent.replace("###ICON###", successIcon);
      break;
      
    case "Info":
      thisContent = thisContent.replace("###ICON###", infoIcon);
      break;
      
    case "Error":
      thisContent = thisContent.replace("###ICON###", errorIcon);
      break;
      
    default:
      
    case "Warning":
      thisContent = thisContent.replace("###ICON###", warningIcon);
      break;
      
    
  }
  toast.innerHTML = thisContent;

  // Insert the toast element as a first child of .toast-container and make sure,
  // that every new toast element will be inserted BEFORE the last one
  toastContainer.prepend(toast);

  // Create Bootstrap Toast instance with some options
  toastInstance = new mdb.Toast(toast, {delay: (type == "Success" ? 5000 : 20000)}); //, {delay: 15000} {delay: (type = "Success" ? 5000 : 20000)}

  // Use show() method to display a toast
  toastInstance.show();
}

function failureHandler(message){
  showToast("Error", message);
}

function successHandler(message){
  showToast("Success", message);
}